...
double p0, p, e, e0, m, mR, dm, A, B=1.0, dB, BR, Bfactor, m1, m2, m3, m4, p1, p2, p3, p4, B1, B2, B3, B4, power,start,end;
double H1, H0, K, W, X, F, V, Dv, gamma, Dp, N;
double DH11,DK1,DW1,DX1,H01,H02,K1,K2,x,X1,X2,Xp1,Xp2,W1,W2,V1,V2,V01,V02,V0;
double w,wr,wi,wcheck, o[3];
double aR,bR,gR,hR,kR,n,Y1,Y2,Z,DZ,DDZ,VZ,Ar1,Ar2,Ai1,Ai2,ar,ai,Ar[2],Ai[2],AM;
int t, q;
double times;
...
for(times=1.0;times>=1.0;times=times-0.5)
{
  ...
  r=r0;
  printf("Please enter the central density:");
  scanf("%lf",&e0);
  printf("Please enter the range of w:\n");
  printf("Start point=");
  scanf("%lf",&start);
  printf("End point=");
  scanf("%lf",&end);
...
r=r-dr;
rx=r+2*mR*log10(r/2/mR-1);

/*-- Matching the solution at infinity --*/

Ar1=2*cos(w*rx)-2*(n+1)/w/r*sin(w*rx)+1/w/w/r/r*(3/2*mR*w*(1+2/n)*sin(w*rx)-n*(n+1)*cos(w*rx));
Ai1=2*sin(w*rx)+2*(n+1)/w/r*cos(w*rx)-1/w/w/r/r*(3/2*mR*w*(1+2/n)*cos(w*rx)+n*(n+1)*sin(w*rx));
Ar2=-2*w*sin(w*rx)-2*(n+1)*cos(w*rx)/r+1/w/r/r*(3/2*mR*w*(1+2/n)*cos(w*rx)+n*(n+1)*sin(w*rx))+(1-2*mR/r)*2*(n+1)/w/r/r*sin(w*rx);
Ai2=2*w*cos(w*rx)-2*(n+1)*sin(w*rx)/r+1/w/r/r*(3/2*mR*w*(1+2/n)*sin(w*rx)-n*(n+1)*cos(w*rx))-(1-2*mR/r)*2*(n+1)/w/r/r*cos(w*rx);
